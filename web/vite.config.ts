...
export default defineConfig({
  ...
  server: {
    port: 5173,
    // Proxy config for when backend is running at localhost:3141.
    // When backend is down, the mockApiPlugin handles requests instead.
    // Uncomment below and remove mockApiPlugin() from plugins when backend is available:
    // proxy: {
    //   '/api': {
